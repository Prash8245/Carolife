...
export default function EmergencyContent(props) {

    const [loading, setLoading] = useState(true);
    const [data,
        setdata] = useState([]);
    const [HosCategory, setHosCategory] = useState("All")
    

    useEffect(() => {
        setdata(props.Data);
        setHosCategory(props.Category);
        console.log(HosCategory);
    } );
            ...
            <section className="text-gray-600 body-font">
                <div className="px-5 py-12 mx-auto">
                    <div className="flex flex-wrap -m-4">
                        ...
                        {data.map((elements) => {
                           if (elements._doc.services.includes(props.Category) || props.Category === "All") {
                            return (
                                <Link
